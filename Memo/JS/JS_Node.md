...
> 扩展:
- null、空字符串""、空数组[]，都可被 Number() 合法的转为0
- undefined、空对象{}、空函数等 Number() 的结果是NaN
...
<!-- 
    let num = 10;
    num++;                      // 11
    let result = num++ + 2;     // num++ 是11(原值), 11+2      -> num=12
    console.log(result);        // 13

    num++ 单独使用都是自加1, 但是连在一起使用时, num++ + 2, num++代表的就是原值, 那自加体现在哪 体现在num上 num=12
...
<!-- 
    * 非数值比较时，会将其转换为Number类型，然后再比较
    * 任何值和NaN比较都是false
    * 如果符号两侧的值都是字符串时，不会将其转换为Number进行比较，而会分别比较字符串中字符的Unicode编码，可以用这种方式排序英文名
        ...
        console.log("abc" < "b"); 
        //比较字符编码时，是一位一位进行比较，abc，先同时比较左侧位，然后比较中间位（右侧没有中间位所以是false），最后比较最后位
    ...
    * 比较两个字符串的数字，可能会得到不可预期的结果，一定要转型，只要把一个数字字符串转为Number类型就可以正常比较了
    ...
    比如：网购时的购物车，可以填写购买的数量，但这时候填写的都是字符串而非数字，时候可能限制用户的购物量不能超过20, 不能超过最大库存，这时候比较可能就会出现问题   
 -->
...
与&&    and     true && false
或||    or      true || false
非!     not     !true           !非运算可以将其他数据类型转换为布尔 !!b
...
<!-- 
    ...
    可以为一个任意的数据类型来取两次反（非运算），来将其转换为布尔值，原理和Boolean()函数一样
...
### 短路运算(逻辑中断)
- 短路运算的原理: 当有多个表达值(值)时, 左边的表达式值可以确定结果时, 就不再继续运算右边表达值的值
...
<!-- 
    ...
    for循环中的三个部分，都是可省略，也可以写在外部
    如果在for循环中，不写任何表达式，只写两个;，这就是个死循环
...
<!-- 
    ...
    总结: 我们可以利用到for循环了i的值, 用if判断语句来写的话 并不是会把for {...} 执行多少次, 而是只输出了 if 里 符合判断的的语句 会一起出来
 -->
...
- 所以实际上 浏览器在函数中传递了3个参数，你想打印那个 直接输出就ok

    你想打印 里面的元素 就
    arr.forEach(function(a,b,c){console.log(a);}); 

    你想打印 里面的索引 就
    arr.forEach(function(a,b,c){console.log(b);});

    你想打印 里面的数组 就
    arr.forEach(function(a,b,c){console.log(c);});
...
> slice( )  
- 提取指定元素
- 不会改变原数组，所以需要创建一个变量来接收 提取指定元素的新数组
...
> join()
- 数组 转 字符串
- 该方法可以将数组转换为一个字符串， 它不会对原数组产生影响, 将转换后的字符串作为结果返回
- 在join()中可以指定一个字符串作为参数，这个字符串将会成为数组中元素的连接符，它默认是使用，号连接的, 还可以输入 空串 或者 &
...
- 可以用来对数组中的元素进行排序
- 如果是单位还可以, 双位会出现问题
- 该方法会对原数组产生影响, 默认按照unicode编码进行排序

    var arr = ["a","b","d","c"]

<!-- 
    arr = [3,4,2,11,5]      
    //11在最前面了，即使对于纯数字的数组，排序使用sort排序，也会按照unicode排，所以对数字, 进行排序时，可能会得到错误的结果 
-->
...
<!-- 
    将新的对象返回
    添加属性后现在这个通过creatPerson方法创建的对象就不是一个空对象了
-->
        return obj；
    }
...
<!-- 
    函数也可以成为对象的属性，如果一个函数作为对象的属性来保存，那我们称这个函数是这个对象的方法，调用函数就是说调用对象的方法（method）, 但是它只是名称上的区别，没有其它的区别 
-->
...
### 静态成员 实例成员
- js的构造函数中可以添加一些成员(构造函数中的属性和方法我们叫成员, 成员可以添加), 可以在构造函数本身上添加, 可以在构造函数内部的this上添加, 通过这两种方式添加的成员, 就分别称为静态成员和实例成员
...
<!-- 
    当创建第一个实例对象的时候 new就开辟了一个空间name age sex还好简单的数据类型 但是 sing是个函数 函数也是对象 也就是说 它还要单独开辟一个空间用来存放这个函数对象
...
<!-- 
    文字描述：
    我们可以理解为Myclass函数对象下面有个属性是prototype, 这个prototype的值为内存空间地址，链接这另一个对象的内存空间，这个另一个对象就是原型对象在函数对象中有一个属性，属性指向着原型对象，每个函数都用而且都是不一样的 
-->
...
> 实例的__proto__属性
- 用Myclass创建了一个对象叫做mc，Myclass函数有个属性叫做protptype，由于mc是由Myclass创建的, 所以mc里面也会有一个属性指向prototype，换句话说mc里面有个隐含属性，也指向ox123, 这个隐含属性，我们可以通过__Proto__来访问该属性

<!-- 也就是说 实例可以通过 __proto__ 来访问 原型对象 -->
...
- 图2：
    通过Myclass创建的mc 对象    指向↗
    隐含属性        ox123

    console.log(mc.__proto__);      //[object Object]
- 而且这个
> mc.__proto__ == Myclass.prototype 意味着它们指向了同一个对象

<!-- 假如 我再创建个 -->
    var mc2 = new Myclass();
<!-- 
    那么由于mc2 也是 通过 Myclass()创建的对象，所有里面也有个__proto__，同样也指向ox123 
-->
...
图1：
    Myclass函数对象                         原型对象（ox123）
    prototype       ox123       指向→       a               123;

    console.log(mc.a);      
<!-- 输出了123，mc里面有a么？没有，它去原型对象里找了 -->
...
- 接下来我们尝试下，向Myclass的原型中添加一个方法：
Myclass.prototype.sayHello=function(){
    alert("hello");
};
<!-- mc里面有sayHello么？ 没有吧 -->
mc.sayHello();      //执行了吧 去原型里面去找了

<!-- 
    以后再出现每一个类都需要用的，我们就添加到原型里, 以后我们创建构造函数时，可以将这些对象共有的属性和方法，统一添加到构造函数的原型对象中，, 这样不用分别为每一个对象添加，也不会影响到全局作用域，就可以使每个对象都具有这些属性和方法了
 -->
...
> hasOwnProperty()
- 可以使用对象的hasOwnProperty()方法来检查，对象自身中是否有这个属性 每个对象都有这么一个方法hasOwnProperty()
...
<!-- 我们创建一个构造函数 -->
    ...
    var mc = new MyClass();
    console.log(mc.name);       //undefined吧

<!-- 接下来向我们的Myclass的原型中添加一个name属性 -->
    function MyClass(){
...
<!-- 
    mc中实际没有name属性，name属性是在原型里的，而我希望检查的是mc自己里有没有name属性 
-->
> hasOwnProperty()
- 可以使用对象的hasOwnProperty()方法来检查，对象自身中是否有这个属性  每个对象都有这么一个方法hasOwnProperty()
    ...
    mc.__proto__ 这个是mc实例访问原型的方式

-----------------------------------

### 原型链
- Star.prototype 是 Star的原型对象, 既然是对象那么就会有__proto__, 
...
- call和apply传递的参数不一样, call传递参数aru1 aru2形式, apply必须是数组的形式[arg]
...
### 继承
- 比如我们在父类中已经写好了很多的方法, 在孩子里直接拿来使用继承父类的方法, 代码就会更简单
...
<!-- 
    ...
    fn.call(o);     // this是o, 如果是fn()调用, 它会是window
 -->

### 借用构造函数 继承父类型属性
- 核心原理:
- 通过 call() 把父类型的this指向子类型的this, 这样就可以实现子类型继承父类型的属性
...
<!-- 
    ...
    console.log(date);      // 返回的是11月 不是10月 比实际月份大1
...
<!-- 
    1/1/1970 0:0:0 这个时间是中国时间（中文系统），-28800000除完后正好是8小时，也就是时差 
-->
...
- 就是封装了一段可被重复调用执行的代码块, 通过此底阿妈快可以实现大量代码的重复使用
<!-- 
    普通对象像一个塑料袋，只是一个容器，而函数对象可以封装一些功能（代码），在需要时可以执行这些功能（代码） 
-->

- 使用typeof来检查一个对象时，会返回function
...
<!-- 把上面的变量中的所有字母提取出来 -->
    ...
    result = str.match(/[A-z]/);    //提取出任意字母 提了个a
...
<!-- 
    setAttributeNode()   设置指定节点的属性 
    可以只设置属性名
-->
<!-- 
    h1.setAttribute('属性名', '属性值')  必须设置两个
 -->
...
<!-- 
    如果获取的样式没有设置，则会获取到真实的值，而不是默认值没有设置width，它不会获取auto，而是一个长度 
-->
...
<!-- 
    通过currentStyle 和 getComputedStyle()读取的样式都是只读的，不能修改 
    如果要修改，必须通过style属性
-->
...
<!-- 接下来要想attachEvent 要on，addEventListener不要on，那传event的时候传不传on, 加上on容易还是去掉on容易 加上on容易吧, 所以这个地方我不要on 那么就有问题了 那ie8不就少个on么？所以在ie8那手动加一个 -->

<!-- 判断obj里是否有addEventListener()这个方法，如有有的话就用它，没有的话用另一个没必要去判断什么浏览器 -->
...
键盘事件一般都要绑定给一些可以获取到焦点的对象，或者是document 文档对象
一般是表单项 或者 document 
...
<!-- 
    ...
    u1.appendChild(li);      //新加进去的超链接 没有点击响应函数 
};
...
<!-- 
    ...
    我们又看不见局部变量 但我们能用闭包的技术能够在外部操作局部变量 相当于间接可以访问, 譬如 我现在在函数内部有一个变量，希望外部能读到这个a 但不希望被改变
 -->
