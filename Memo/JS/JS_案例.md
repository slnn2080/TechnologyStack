...
<!-- 
    ...
    但是假如除到9发现已经等于0 则10-17之间的数字就不用继续计算了
    所以写上break能大大减少计算步骤  
-->
...
<!-- 
    思路1: 因为newArr.length可以动态监测数组里的元素, 让筛选出来的数组添加到新数组中的最后一位
    ...
    newArr[i] 不能这么写 因为 当值为77的时候满足条件 此时的i为4 也就是
    newArr[4] = arr[4]; 造成了 0 1 2 3 为empty

    我们的newArr[0] 应该从0开始 依次递增


    思路2: 既然应该从0开始 我们在外面定义一个变量j=0
    newArr[j] = arr[i]
    j++
...
<!-- 但是还想循环，也就是当是最后一张的时候再点跳到第一张,   当是第一张的时候再点跳到最后一张 -->
...
<!-- 如果你img大于对最大索引了 就让你等于第一张  -->
        img.src = imgArr[index];
    };

<!-- 这样的话基本上就完成了，接下来我们优化一下，最起码翻起来我想知道翻到了第几张而且这么循环翻起来 我也不知道一共有几张图片所以我希望有这么一个效果 -->
...
<!-- 先不看下面的for循环 这个是什么效果？只要我一点items 上来我就把上面的 全选/全不选按钮选中了 -->
    ...
    if(items[j].checked){
<!-- 如果items[j].checked为ture能不能说明4个被全选？不能吧
如果为true只能说明有一个被全选 得4个全为true才能证明被全选吧
那就麻烦了，你得4个都看一遍才能知道是不是全选吧，那应该怎么改
那换个想法，只要有一个没选中则就不是全选吧 -->
    }; 
    
    改成：

    if(!items[j].checked){
<!-- 一旦进入这个判断则证明 不是全选状态，那我能干什么？那我应该把checkedAllBox设置成false吧
将checkedAllBox设置为选中状态 -->
...
<!-- 思考：
我的知道点的是哪个吧？我的响应函数是给allA绑定的吧，我要是点击第一个delete是应该触发第一个响应函数吧
既然是响应函数 就应该有 this 吧 用this的话 3个delete点哪个就是哪个吧

响应函数里的this就是你绑定那个事件的函数 给谁绑定就是谁, 点谁this就是谁

有this之后了 我们就要考虑 我点delete删除的是所在行的tr, 那现在我们只有a 我能不能通过a 获取到 tr tr和a是什么关系，爷爷 孙子的关系 所以我们可以先获取a的父元素 td 然后再获取td的父元素 tr -->
...
<!-- 左37 上38 右39 下40 -->
<!-- 用if(event.keyCode === 37){}可以 但是有些麻烦 我们观察下，是不是都在做全等 === 的判断所以这里用switch case -->
...
<!-- 
    只要是页面网上滚动 就会有 scrollTop 值 根据这个值就知道什么时候到banner了
    比如scrollTop:100, 当等于100的时候 就知道banner到顶了
 -->
...
<!-- 
    思考：
    首先 我得知道鼠标动没动吧，鼠标一动修改box1的位置
    是onmousemove事件吧, 那onmousemove事件绑定给谁呢？绑定给box1行不行？试试
 -->
...
<!-- 但是div不动 设置偏移量只对什么起作用了？只对开启定位的元素起作用, 所以要开启div的定位功能
但是只能往一个方向走，为什么？因为事件是给box1绑定的，所以只有鼠标在box1里移动的时候才能触发啊离开box1的范围就不走了所以给box1绑定是个错误的决定 给文档绑定 所以把onmousemove事件 绑定给 document -->

    document.onmousemove = function(){};

<!-- 实现了 但是有没有什么问题 当页面过高之后，会出现滚动条，这时如果拖动滚动条 会出现 鼠标和 div 分离的情况 能分离的范围就是 就是滚动的垂直偏移量
为什么呢？我们用了
clientX
clientY
...
<!-- chrome认为浏览器的滚动条是body的，可以通过body.scrollTop来获取, 火狐等浏览器认为滚动条是html的因为我是给body设置了一个高度1000px 然后html没装下才出现的滚动条，html是父元素 子元素body溢出 -->
...
<!-- 现在距离有了 剩下把这段距离 加给垂直偏移量 设置div的偏移量 -->
    box1.style.left = left + sl + "px";
    box1.style.top = top + st + "px";

<!-- 上面是垂直的问题，水平也有问题 自己照着改 -->
};
};
...
<!-- 
    ...
    title.addEventListener('mousedown', function(event) {
        ...
        或者 
        这里想表达的是add这种方式移出onmouseup事件怎么移除
        document.addEventListener('mouseup', remove);
            function remove() {
...
<!-- 
    不能给元素对象绑定 鼠标抬起事件, 因为当鼠标移动到别的元素上时 会出现放置不了的情况
    因为 鼠标松开事件 是绑定给box1的 当移动到box2上时，松手后还会动 因为在box2上松手是触发box2的松手事件 1的没有 所有会跟着动 所以不能给box1绑定
    得给document绑定 改后状态
-->
...
<!-- 
    ...
    先思考为什么box会跟着动，是因为绑定了onmousemove事件吧，那让它停是不是取消onmousemove事件就可以了 
-->
...
<!-- 
    那现在还有问题，我现在是把onmousemove取消了，那onmouseup怎么办 它还在不在 
-->
    document.onmouseup = function(){
...
<!--
    alert(鼠标松开了);
    可是我发现 在box1范围外的地方，再点击屏幕 再松开还是会触发alert
    此时此刻
    document.onmouseup = function 这个事件也没有意义了  所以这个事件我们应该也给取消掉
-->
   

<!-- 取消dcoument的onmouseup事件 -->
document.onmouseup = null;
<!-- 
也就是说不管 onmousedown还是onmouseup都是在鼠标按下有意义，松开时没有意义都应该被取消掉 
-->
};
};
...
> 修改鼠标点击位置 和 元素的位置
<!-- 
    现在的拖拽效果都是在box1的左上角，那怎么才能点哪拖哪呢？那点到一个地方之后，想改成点哪拖哪的状态
    所以要移动box的位置到点击点，那移动多少？
    向上挪动        修改top值 减去
    向左挪动        修改left值 减去
 -->
...
event = event || window.event;
<!-- 上面得出的数字减掉    var left = event.clientX - ol; -->
...
<!-- 那再回过头想想 当我拖拽的时候 别的东西会一起动的原因是 onmousedown的事件传递给了别的东西 那我这么想 当我拖拽box1的时候 我让box1变成强盗 不管拖谁 都是拖我 这样所有的事件都会揽到自己身上 就可以了
...
<!-- 
    接下来需要知道滚轮滚没滚吧，那给谁绑定？是不是当在div上的时候 开始吧
    所以给box1绑定鼠标滚轮滚动事件
 -->
...
<!-- 
    接下来我们思考下，我们在ie中触发的alert函数 和 在火狐中触发的alert函数 是一个么？不是吧, 在ie中是触发的上面的函数吧，火狐里触发的是下面的函数吧
    这两个函数用不用做两个业务？业务需求都是一样的吧, 我们可以这么改：
 -->
...
<!-- 赋值给了box1.onmousewheel
然后我将box1.onmous ewheel 作为回调函数传递给了 bind 所以是同一个函数 -->
...
<!-- 但是ie8 又出现了问题,ie8不支持，event.preventDefault(); 如果直接调用会报错 -->
...
<!-- 
    这样写行，但是我更想把这个函数写成通用的方法 这样让它适应各种动画的操作 既然是想适用任何动画的操作 那是不是只修改left的值？不是吧 有可能我要修改top width height 有没有一个方法是根据样式名来获取样式值的？
    插入这个函数  这个函数可以获取当前任意的样式，也是当前所显示的样式 之后这么改 
-->
...
<!-- 
    二，我现在的动画是给box1加的 以后可能还要给box2加等等 这东西不是定的 
    所以
    定义一个参数 obj： 执行动画的对象
...
报错是callback is not function

<!-- 因为btn01 我没有传递回调函数，所以值是一个undefined 把undefined当做回调函数一调就报错了 -->
...
<!-- 接下来完善一下功能, 第一个 点击导航点切换指定图片, 点击第一个 显示第一张。。。。。。 -->
...
<!-- 此时是第6张 我们当成第一张看的 但是导航点一个都没有变黑？
为什么？因为第6张索引为5 但是我们没有索引为5的超链接 所以一个没变
当第6张的时候 应该第一个变黑吧 -->
...
- 解析:
- 我们的每一个菜单都是一个div, 当div具有collapsed这个类时，div就是折叠的状态, 当div没有这个类的时候，div就是展开的状态. 实际上 想叠起来还是展开就是对这个类来进行操作

<!-- // 点击菜单，切换菜单的显示状态 -->
<!-- 
    我们要先找到菜单，并绑定单击响应函数吧，那应该绑定给谁呢？
    一个是<div class="collapsed">
    一个是<span class="menuSpan">支持我们</span>
    
    绑定给div行，但是有问题，点div的整个区域都能触发 div的响应函数 也就是说 我点里面的内容也会 触发div的函数, 所以应该绑定给span
-->
...
<!-- this代表我 当前点击的span -->
        this
<!-- 
    但是我们要获取的是div吧，如果我点的这个span 我获取的是这个div 获取当前span的父元素 parentDiv这个div就是我们点的div也就是我们要操作的div吧 
-->
        let parentDiv = this.parentNode；

<!-- 
    接下来点span 把整个div折叠起来 其实就是添加一个 折叠起来的collapsed这个类 
-->
<!-- // 关闭parentDiv -->
        addClass(parentDiv, 'collapsed')

<!-- 
    但是有问题 这样的话 打开后 合不上了 所以addClass()不行 用add永远是添加 用remove()永远是删除,我更希望你 打开的给你关上，关的给你打开, 所以要用toggleClass() 切换类，有的话删除 没有的话 加上

    但是也有个问题 我希望在打开一个的同时 把上一个关闭, 也就是在我打开第二个的时候 关闭 第一个
    打开以后菜单以后，应该关闭之前打开的菜单 关闭好关，直接在前面的那个添加一个关闭的class, 但问题是关谁啊？ 也就是我之前打开的菜单是谁啊？ 

    我在外面外面定义一个变量 来保存当前打开的菜单   为什么要设置这个 因为一上来默认就是这个打开

 -->

函数最外部
    let openDiv = menuSpan[0]
<!-- 这样写是第一个吧 这是第一个span吧 我要找的是第一个div -->

    let openDiv = menuSpan[0].parentNode    //这个就是第一个打开的div

<!-- 在打开新的div的时候 要关闭第一个div吧 -->
    addClass(openDiv, 'collapsed');

<!-- 
    但是再下一轮的时候 第一个div变成第二个了吧 也就是新打开的变成上一个，下一个要打开的要关闭上一个, 也就是说 上一个是变化的 
-->

    let openDiv = menuSpan[0].parentNode 
<!-- 所以像上面这么写 写死了，永远上一个div是第一个div -->

        
    let parentDiv = this.parentNode；（参考用）
<!-- 所以加下来要修改openDiv为当前打开的菜单 -->
    openDiv = parentDiv;

> 这里可能解决困扰我多年的问题
<!-- 
    还是有问题，关闭后 打不开了, 为什么？ 一上来第一个div没有关闭的class，我点它下 添加了这个class的类 此时我的openDiv 和 parentDiv 是同一个div 刚去掉class 就又加上了 那addclass什么时候加啊 如果openDiv 和 parentDiv是同一个的话 这个就不用动了
-->


<!-- 判断 openDiv 和 parentDiv 是不是相同 -->
        if(openDiv != parentDiv){
            addClass(openDiv, 'collapsed');
        }
<!-- openDiv 和 parentDiv 不一样 我才加 一样我就不管了你了 -->


>过渡效果
- 展开需要加一个动画，折叠需要加一个动画

<!-- 为了统一处理动画过渡效果，我们希望在这将addclass改为toggleclass -->
        addClass(openDiv, 'collapsed')
        ↓
        toggleClass(openDiv, 'collapsed')

<!-- 
    但是有问题，一次性的打开了两个，为什么？因为又把关闭的类移除了,因为toggleClass是有就给你移除 没有就添加,但是这里toggleClass 我不希望它有移除的功能 它唯一需要做的工作就是给我加上,不用你实现移除的功能,也就是说openDiv里有这个类了 你就不进判断就行了不是么,所以在上面 再加个判断
-->
...
<!-- 接下来是点击后 把展开的部分折叠起来，折叠的是谁？div？span? div吧 span就那么一行没办法叠 它叠是把整个div叠起来-->
        
> 接下来做动画的过渡效果
<!-- 
    慢慢悠悠的展开 慢慢悠悠的合上, 那就说明这个动画有两个位置，是A → B 修改高度值 A的高度是整个div的高度，B的高度是span的高度, A → B的过渡 就是在两个高度之间做过渡效果, A是谁 B是谁？
-->

<!-- 先获取A 和 B的值, 在切换类之前 获取元素的高度 A div展开时的高度 -->
let begin = parentDiv.offsetHeight;

<!-- 切换parentDiv的显示 -->
toggleClass(openDiv, 'collapsed')

<!-- 在切换类之后 获取元素的高度 B div关闭时的高度 -->
let end = parentDiv.offsetHeight;

console.log(begin, end);    222 25

<!-- 那我要执行过渡效果就是从222 向 25去过渡 begin是开始的值 end是结束的值 -->

<!-- // 动画效果  就是高度从begin到end去过渡 -->
<!-- // 将我们的高度重置为begin 因为222 -- 25 叫过渡，25 --- 25咋过渡 -->
parent.style.height = begin + 'px'; 
<!-- // 这个是个内联样式，优先度比class高 这样让它可以慢慢悠悠执行动画了 -->
    
<!-- 执行动画 从begin到end 执行过渡 -->
move(parentDiv, 'height', end, 10, function(){

});

<!-- 
    执行动画后关不上了，过渡完后 内联样式里 留了一个height25px 优先级最高 打不开了
    设置内联样式就是为了执行动画，现在动画执行完了 内联样式就不需要了吧
    删了吧，动画执行万了后删除，怎么知道动画执行万了？
 -->

move(parentDiv, 'height', end, 10, function(){
<!-- // 动画执行完毕，内联样式没有存在的意义了 删除它 -->
    parentDiv.style.height = '';
});
<!-- 回调函数吧，当回调函数执行完了后 动画就执行完了吧, 展开的ok了 关闭再加一个  -->
