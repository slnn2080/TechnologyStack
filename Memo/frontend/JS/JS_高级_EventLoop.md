...
# 宏任务队列
同步代码会放到主线程中立刻执行, 异步代码会放到队列中**等待同步代码执行完毕后**再执行
```js
setTimeout(function() {
    console.log(1)
}, 0)
...
```
那也就是说 当定时器的时间到了之后 才会放到任务队列中 等待下一次的轮询 从任务队列中拿到主线程里面执行
```
```js
...
**要点**: new Promise() 括号中的代码是会立即执行的 这个部分的代码是**同步代码**
