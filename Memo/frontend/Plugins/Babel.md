...
先週　私がローカルで新しいnuxtプロジェクトを立ち上げて、swiperについてのことを試してます
mountedのサイクルのロジックが実行されてないことを発見して、そして池田さんがnpm generateとnpm startでプロジェクトを立ち上げて、問題がなくなりました。

でも同じ操作同じコードでkintoのプロジェクトであまり効かないんです。
そしてieでscript1022構文エラーが出てきたので、おそらく
1. es6の構文はieは知らないかな
<!-- 
  プロジェクトがbabelを配置しましたので
 -->  
...
競合するプラグインを探したり、swiperのバージョンを下げてみたりできるようになりましたが
バージョンを下げると問題が増えたら、えよりも損の方が大きいかな