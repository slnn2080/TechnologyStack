...
### 自我简介
- hello, 大家好, 我叫jiangjiangjiangjiang
- 在未来的一段时间里 我给大家带来一套scss教学视频 希望大家能够喜欢 网上的scss视频不少 你说为什么我又出一套
- 要知道一道菜不可能满足所有人的胃口 萝卜青菜各有所爱 兴许我就是你的菜呢是不

- so 在接下来的一段时间了 我会带领大家认识我 附带给大家讲解下scss
- 本套课程中

  - scss的基础语法
  - scss的实战教程

- 这套视频里给大家整理了一套实战 我们通过敲代码来巩固前面所学到的知识
  - 一方面是查漏补缺
  - 另一方面是看看在实战中scss的 应用方式 和 要点

- 另外这套课程的受众人群 我希望是只学习了html css的小伙伴们也能听懂
- 所以在整套scss中 讲到类似 if for语句的地方 可能会稍微说的多一些 望见谅 别说这哥们真啰嗦哈哈

- 好啦 最后
- 希望这套课程大家能够喜欢, 开心学scss, 悄悄变强大

-------------------

## Sass的简介:
我觉得大家会在某个阶段接触到scss 比如
  1. 已经工作的小伙伴们在接触公司项目的时候 发现项目里面使用的scss
  2. 在报班或自学的过程中接触到了scss

不管是哪种情况 在大家去搜scss的相关教程的时候 肯定会搜到 sass 和 scss 相关信息, 这里有没有小伙伴比较迷糊
它俩是个啥? 嘛关系? 学哪个？  


极有可能是一脸懵逼的状态 这节课带大家简单的了解一下 sass 同时也说说他们之间的一个区别

说到sass那就简单的说提下haml sass算是haml的一个部分 haml也好还是sass也好 初衷啊都是为了偷懒 简写或少写代码
这都是由一群大大们开发出来的 当时吧他们觉得写html太麻烦了 还得记得闭合标签 费劲

既然这么费劲 那就怎么舒服怎么来 按照自己的编码习惯 开发一门语言不就好了么 所以haml的诞生了 它可以用一套新的语法规则 还写html代码 我们可以说haml是html的模版引擎 预处理器 或者就在直接点 理解为 html的另一种简洁 且 优雅的写法 haml是简写html的是么 css呢？sass就是用来简写css的 我们简单来体会一下 haml 的写法

因为我们要使用 haml 的话还需要进行一些配置 这里咱们又不专门讲它 没必要 我写写 大家看看差别就可以了

```html
<!-- 我们会发现 haml 使用的是缩进的语法格式 同时不用写闭合标签是么 -->
<p>你好, sam</p>
...
我们会发现它使用了一些预定的符号来代替标签 使用了缩进语法来表示子元素 除了看的不太习惯 确实让html代码简洁了很多 其实要真写多了你也会觉得haml写起来很舒服
...
后续我们都会陆续的进行讲解 这节课我们也时候简单的来体验下 sass 的写法
...
有没有发现 简洁了很多 这种缩进的形式还好 结构清晰 一套缩进的语法 就能代替我们以前要写两套样式
但是没括号 没分号 感觉还是有些别扭 所以sass为了让熟悉css的人能够更加的平稳的过渡到sass

在sass 3.0之后将 将 .sass 结尾的文件 改成 .scss 结尾
