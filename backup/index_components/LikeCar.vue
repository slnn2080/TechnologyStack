...
export default {
  ...
  mounted() {
    ...
    Promise.all([carGet(), delivGet()])
      .then(function (res) {
        const likes = localStorage.getItem("likecar")
        const likesArray = JSON.parse(likes)
        const allCar = res[0].data
        const delivData = res[1].data.data
        console.log(allCar)

        let likesCars = likesArray.map(item => ({
          ...allCar.find(({ car_model_code }) => item == car_model_code),
          ...item,
        }))

        let carMaster = likesCars.map(item => ({
