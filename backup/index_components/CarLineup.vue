              ...
              <div class="description">
                ...
                <p class="price">
                  ...
                  <span class="price-number">
                    {{ Number(item.yearmax_bonusmax_taxin).toLocaleString() }}
...
export default {
  ...
  mounted() {
    ...
    Promise.all([carGet(), delivGet()])
      .then(function (res) {
        const rankingcar =res[0].data.filter((i) => i.ranking_now.length > 0)
        const delivData = res[1].data.data

        let carMaster = rankingcar.map(item => ({
...
.item {
  ...
  .car-image {
    ...
    .image-block  {
      img {
        height: auto;
        transform: scale(0.7) translate(-72vw,-20vw);
